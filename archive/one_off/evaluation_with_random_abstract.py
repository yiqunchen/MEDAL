...
# Sample 10 questions for testing
# question_data = dict(list(question_data.items())[:10])

# ==== Assign a random unrelated abstract to each question ====
all_abstracts = list(abstract_data.values())
for qid, item in question_data.items():
    random_abstract = random.choice(all_abstracts)["abstract"]
    item["abstract"] = random_abstract
...
Here is a abstract from a Cochrane review that you may refer to if it is relevant:

\"\"\"{abstract}\"\"\"

If this abstract is not helpful, answer the question based on your knowledge and reasoning.
...
"""
    ...
    pbar = tqdm(total=len(tasks), desc="Evaluating (random abstract)")
...
async def main():
    ...
    with open("/Users/pql/Desktop/proj/rag_eval_results_with_random_abstract.json", "w") as f:
