...
# ==== Load Pre-Saved Data with PubMed Context ====
...
async def evaluate_questions_parallel(question_data: Dict[str, dict], client, model="gpt-4o-mini", max_concurrent=6):
    ...
    async def query_llm(qid: str, item: dict):
        ...
        abstract = item.get("pubmed_context", "")
...
Here is a set of PubMed abstracts that may be helpful:

\"\"\"{abstract}\"\"\"

If this evidence is not helpful, answer based on your clinical reasoning.
"""
        try:
    ...
    pbar = tqdm(total=len(tasks), desc="Evaluating with PubMed context")
...
async def main():
    ...
    with open("/Users/pql/Desktop/proj/rag_eval_results_with_pubmed_search.json", "w") as f:
